...
# surival functions
s_est_phase <- function(w, w0) {
    return(s * (w/w0)^(-k/w0))
}

s_densindep_phase <- function(w, w0) {
    return(s * D^(q - k/w0) * (w0/w)^(q))
}

s_ST_bound <- function(w, w0, wa, lri) {
    return((w/wa)^(-r)/n(w0, lri))
}

# survival to adulthood - for resident only
s_adult <- function(w0, wa, wt) {
    return(s * D^(q - k/w0) * wt^(r - q) * w0^q * wa^(-r))
}
...
s_mut <- function(w, w0.mut, w0.res, wa, lri) {
    ...
    for (i in 1:LEN) {
        ...
        if (W[i] > WA[i])
            surv[i] <- 0 else if (W[i] <= D * W0.MUT[i]) {
            # establishment phase
            surv[i] <- s_est_phase(W[i], W0.MUT[i])
        } else {
            # excess juveniles - crash at onset of thinning
            if (WT_mut[i] < D * W0.MUT[i]) {
                s_crash <- s_ST_bound(D * W0.RES[i], W0.RES[i], WA[i], LRI2[i])/s_est_phase(D *
                  W0.RES[i], W0.RES[i])  # magnitude of crash
                surv[i] <- s * D^(-k/W0.MUT[i]) * (s_crash/alpha_ij(W0.MUT[i], W0.RES[i])) *
                  (W[i]/D/W0.MUT[i])^(-r * alpha_ij(W0.MUT[i], W0.RES[i]))
                ...
                if (W[i] <= WT_mut[i]) {
                  ...
                  surv[i] <- s * D^(-k/W0.MUT[i]) * (gam[i] * WT[i]/D/W0.RES[i])^(-q) *
                    (W[i] * W0.RES[i]/WT[i]/W0.MUT[i]/gam[i])^(-r * alpha_ij(gam[i] *
                      W0.MUT[i], W0.RES[i]))
                }
            }
        }
...
gamma <- function(w0.mut, w0.res, wa, wt.res) {
    gam <- (((wa/wt.res)^0.25 * (1 - (w0.res/w0.mut)^0.25) + (wa/w0.mut)^0.25 - g^0.25)/((wa/w0.res)^0.25 -
        g^0.25))^4
...
dR0 <- function(w0, wa, lri) {
    return((R0(w0 * (1 + DERIV), w0, wa, lri) - R0(w0 * (1 - DERIV), w0, wa, lri))/(w0 *
        2 * DERIV))
}
...
ESS_numeric2 <- function(wa, lri, delta, max_counts) {
    ...
    for (i in 1:length(wa)) {
        ...
        # check to see if optimum is greater or lower seed size increase seed mass
        if (der > 0) {
            flag <- 1
        } else {
            # decrease seed mass
        ...
        while (abs(der) > TOL && count < max_counts) {
            ...
            # print( c(count, res, dR0(res[i], wa[i], lri[i]), mult)) increment too large -
            # overshoots optimum
            if (der * flag < 0 || W0_next > wa[i]) {
...
# competition effect
alpha_ij <- function(w1, w2) {
    return(alph_c/(1 + (w1/w2)^alph_k))
}

# plot survival curves Create a plot of A-Ci curves showing Aj and Av
Plot.Surv <- function(w0, wa, lri) {
    WT <- wt(w0, wa, lri)

    plot(function(x) s_est_phase(x, w0 = w0), w0, D * w0, lwd = 2, col = "red", xlab = "W (kg) [log]",
        xlim = c(w0/2, wa * 2), ylim = c(1e-04, s * 2), ylab = "Survival (0-1)",
        log = "xy")
    curve(s_densindep_phase(x, w0 = w0), from = D * w0, to = max(min(WT, wa), D *
        w0), lwd = 2, col = "blue", add = TRUE)
    curve(s_ST_bound(x, w0 = w0, wa = wa, lri = lri), from = min(max(WT, D * w0),
        wa), to = wa, lwd = 2, col = "green", add = TRUE)
    curve(s_mut(x, w0.mut = w0, w0.res = w0, wa = wa, lri = lri), from = w0, to = wa,
        lwd = 1, col = "black", add = TRUE)
    # points(c(W0, WA), c(s_est_phase(w0,w0), s_combined(wa, w0,w0,wa)), type = 'p',
    # cex = 1.5, ) plot(c(W0, WT, WA), c(s_est_phase(W0,W0), s_thinning_phase(WT,
    # W0,W0,WA), s_thinning_phase(WA, W0,W0,WA)), type = 'p', cex = 1.5, log='xy')
    legend("bottomleft", leg = c("establishment", "dens indep", "thinning", "all"),
        lwd = c(2, 2, 2, 1), col = c("red", "blue", "green", "black"))
}
...
ll_plot <- function(X, Y, XLAB, YLAB) {
    ...
    r2 <- cor(log10(Y), log10(X))^2
    n <- length(X)
    leg <- c(paste("B = ", format(fit$b, digits = 4)), paste("Int = ", format(Int,
        digits = 4)), paste("n = ", n), paste("R2 = ", format(r2, digits = 3)))
    legend(x = "topleft", legend = leg, lty = 0, cex = 1)
}
...
PIP <- function(low, up, wa) {
    ...
    wireframe(r ~ log10(x) * log10(y), scales = list(arrows = FALSE), perspective = F,
        drape = T, colorkey = TRUE, screen = list(z = 90, x = 0, y = 0))
}
