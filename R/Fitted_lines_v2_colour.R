...
postscript("plots/fig3-2.eps", horizontal = FALSE, onefile = FALSE, height = 6, width = 6, 
...
plot(X, Y, type = "p", log = "xy", axes = F, ann = F, ylim = c(min(Y_ax), max(Y_ax)), 
    xlim = c(min(X_ax), max(X_ax)), col = "red", pch = 19, cex = 1.2, xaxs = "i", 
    yaxs = "i", family = "helvetica")
...
b_LRI <- 0.9519
s <- 0.944
points(X, ESS_analytic(X, LRI(X)), type = "l", lty = "solid", lwd = 2, col = "red")

# LOAD PLANT DATA
X <- obs.plt$Wa
Y <- obs.plt$W0
points(X, Y, type = "p", pch = 17, cex = 1.2, lwd = 0.25, col = "darkgreen")

D <- 2
b_LRI <- 0.632
s <- 0.00677/17
points(X, ESS_analytic(X, LRI(X)), type = "l", lty = "solid", lwd = 2, col = "darkgreen")

# 1:1
points(c(10^-7, 10^5), c(10^-7, 10^5), type = "l", lty = "dashed", lwd = 1.5)

# add plots for other taxa
points(X, 2.1e-06 * X^0.31, col = "red", type = "l")
points(X, 0.0014 * X^0.89, col = "green", type = "l")
points(X, 0.0075 * X^0.42, col = "blue", type = "l")
points(X, 0.053 * X^0.77, col = "cyan", type = "l")
