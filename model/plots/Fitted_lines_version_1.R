#MS FIG 1
	postscript("plots/fig3.eps", horizontal=FALSE, onefile=FALSE, height=6,width=6,pointsize=10, paper ="special" )
	#LOAD MAMMAL DATA
	...
	#LOAD PLANT DATA
	points(obs.plt$Wa, obs.plt$W0, type="p", pch=19, cex=1.0, col="blue")

	#------------------------------------------------------------
	#ADD FITTED CURVES - ver 2 --> use different values of D
	X = seq.log(10^-7, 10^5, 2);
	c_LRI = 0.927/2;
	b_mam = 0.9519; b_plt= 0.632; b_step= (b_mam/b_plt)^(1/10);
	s_mam =0.944; s_plt = 0.00677/17; s_step= (s_mam/s_plt)^(1/10);
	...
	while(i<4)
		...
		s = s_mam*s_step^(i-3);
	     points(X, ESS_analytic(X, LRI(X)), type="l", lty="solid", lwd=1.0, col="red")
	     i=i+1;}

	#best fit
	b_LRI = b_mam; s = s_mam;
	points(X, ESS_analytic(X, LRI(X)), type="l", lty="solid", lwd=1.0)
	...
	while(i<16)
		...
		s = s_plt*s_step^(i-8);
		X2<- X[1:(N/2)]
		points(X2, ESS_analytic(X2, LRI(X2)), type="l", lty="solid", lwd=1.0, col="blue")
	      X2<- X[(N/2):(N+1)]
		points(X2, ESS_analytic(X2, LRI(X2)), type="l", lty="solid", lwd=1.0, col="blue")
	     	i=i+1;}
	...
	b_LRI = b_plt; s = s_plt;
	X2<- X[1:(N/2)]
	points(X2, ESS_analytic(X2, LRI(X2)), type="l", lty="solid", lwd=1.0)
	X2<- X[(N/2): (N+1)]
	points(X2, ESS_analytic(X2, LRI(X2)), type="l", lty="solid", lwd=1.0)
	#1:1
	points(X, X, type="l", lty="solid", lwd=2.0);
