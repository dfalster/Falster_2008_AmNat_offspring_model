#MS FIG 1
	...
	plot(X, Y, type="p",log="xy", axes=F,ann=F, ylim=c(min(Y_ax), max(Y_ax)),xlim=c(min(X_ax), max(X_ax)),
		col="red", pch = 19, cex=1.2, xaxs="i", yaxs="i", family="helvetica")
  ...
  points(X, ESS_analytic(X, LRI(X)), type="l", lty="solid", lwd=2, col='darkgreen')
	...
	points(X, 2.1E-6*X^0.31, col='red',  type="l")
	points(X, 1.4E-3*X^0.89, col='green', type="l")
	points(X, 7.5E-3*X^0.42, col='blue',  type="l")
	points(X, 5.3E-2*X^0.77, col='cyan',  type="l")
